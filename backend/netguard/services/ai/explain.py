...
def explain(f: Finding, *, audience: str = "beginner", use_ai: bool = True) -> dict[str, Any]:
    ...
    try:
        reply = llm.complete(
            SYSTEM_PROMPT % audience, _prompt(f), max_tokens=2000, effort="medium", json_mode=True
        )
        ...
        return result
    result.update(clean)
    result.update(source="ai", model=reply.model, provider=getattr(reply, "provider", None),
